package BarinakSistemi2;
 
import java.util.Date;
import java.util.List;
import java.util.Scanner;
 
public class MenuIslemleri {
 
	public static int basvuruSayaci = VeritabaniIslemleri.sonBasvuruIdGetir();
 
	// ═══════════════════════════════════════════════════════════════════════════
	// YÖNETİCİ MODU
	// ═══════════════════════════════════════════════════════════════════════════
 
	public static void yoneticiModu(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- YÖNETİCİ MODU ---");
 
		Calisan aktifCalisan = calisanDogrula(scan, yonetici);
		// Yetki kontrolü
		aktifCalisan.hayvanEkleYetkisi();
 
		System.out.println("\nHoş geldin " + aktifCalisan.getName() + " (Yönetici)");
		System.out.println("\n1-) Hayvan Ekle");
		System.out.println("2-) Başvuru Onayla / Reddet");
		System.out.println("0-) Ana Menüye Dön");
		System.out.print("Seçiminiz: ");
 
		int secim = scan.nextInt();
		scan.nextLine();
 
		switch (secim) {
			case 0:
				return;
			case 1:
				hayvanEkle(scan, yonetici, aktifCalisan);
				break;
			case 2:
				basvuruYonet(scan, yonetici, aktifCalisan);
				break;
			default:
				throw new BarinakIstisnasi("Geçersiz seçim!");
		}
	}
 
	// ═══════════════════════════════════════════════════════════════════════════
	// VETERİNER MODU
	// ═══════════════════════════════════════════════════════════════════════════
 
	public static void veterinerModu(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- VETERİNER MODU ---");
 
		Calisan aktifCalisan = calisanDogrula(scan, yonetici);
		// Yetki kontrolü
		aktifCalisan.saglikKaydiYetkisi();
 
		Veteriner veteriner = (Veteriner) aktifCalisan;
 
		System.out.println("\nHoş geldin Dr. " + veteriner.getName()
				+ " (Veteriner | " + veteriner.getUzmanlikAlani() + ")");
 
		if (yonetici.tumHayvanlariListele().isEmpty()) {
			System.out.println("\nBarınakta hayvan bulunmamaktadır.");
			return;
		}
 
		System.out.println("\n--- Barınaktaki Hayvanlar ---");
		for (Hayvan h : yonetici.tumHayvanlariListele()) {
			System.out.println("- ID: " + h.getAnimalId() + " | İsim: " + h.getName()
					+ " | Tür: " + h.getTur() + " | Sağlık: " + h.getSaglikDurumu());
		}
 
		System.out.print("\nİşlem yapmak istediğiniz hayvanın ID numarası: ");
		int hayvanId = scan.nextInt();
		scan.nextLine();
 
		Hayvan hedefHayvan = yonetici.hayvanBul(hayvanId);
 
		System.out.println("\n1-) Sağlık kayıtlarını görüntüle");
		System.out.println("2-) Yeni sağlık kaydı ekle");
		System.out.println("0-) Ana Menüye Dön");
		System.out.print("Seçiminiz: ");
 
		int islem = scan.nextInt();
		scan.nextLine();
 
		switch (islem) {
			case 0:
				return;
			case 1:
				veteriner.saglikKayitlariniGoruntule(hedefHayvan);
				break;
			case 2:
				saglikKaydiEkle(scan, yonetici, veteriner, hedefHayvan);
				break;
			default:
				throw new BarinakIstisnasi("Geçersiz seçim!");
		}
	}
 
	// ═══════════════════════════════════════════════════════════════════════════
	// BAKICI MODU (sadece görüntüleme)
	// ═══════════════════════════════════════════════════════════════════════════
 
	public static void bakiciModu(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- BAKICI MODU (Görüntüleme) ---");
 
		Calisan aktifCalisan = calisanDogrula(scan, yonetici);
 
		System.out.println("\nHoş geldin " + aktifCalisan.getName()
				+ " (" + aktifCalisan.getRole() + ")");
		System.out.println("\n1-) Hayvanları görüntüle");
		System.out.println("2-) Müşterileri görüntüle");
		System.out.println("3-) Çalışanları görüntüle");
		System.out.println("4-) Tüm başvuruları görüntüle");
		System.out.println("0-) Ana Menüye Dön");
		System.out.print("Seçiminiz: ");
 
		int islem = scan.nextInt();
		scan.nextLine();
 
		switch (islem) {
			case 0:
				return;
 
			case 1:
				if (yonetici.tumHayvanlariListele().isEmpty()) {
					System.out.println("Barınakta hayvan yok.");
				} else {
					System.out.println("\n--- Hayvanlar ---");
					for (Hayvan h : yonetici.tumHayvanlariListele()) {
						System.out.println("- ID: " + h.getAnimalId() + " | " + h.getName()
								+ " | " + h.getTur() + " | Sağlık: " + h.getSaglikDurumu());
					}
				}
				break;
 
			case 2:
				if (yonetici.tumMusterileriListele().isEmpty()) {
					System.out.println("Kayıtlı müşteri yok.");
				} else {
					System.out.println("\n--- Müşteriler ---");
					for (Musteri m : yonetici.tumMusterileriListele()) {
						System.out.println("- " + m.getInfo());
					}
				}
				break;
 
			case 3:
				System.out.println("\n--- Çalışanlar ---");
				for (Calisan c : yonetici.tumCalisanlariListele()) {
					System.out.println("- " + c.getInfo());
				}
				break;
 
			case 4:
				if (yonetici.tumBasvurular().isEmpty()) {
					System.out.println("Başvuru kaydı yok.");
				} else {
					System.out.println("\n--- Tüm Başvurular ---");
					for (SahiplenmeBasvurusu b : yonetici.tumBasvurular()) {
						String musteriAdi = (b.getMusteri() != null) ? b.getMusteri().getName() : "?";
						String hayvanAdi = (b.getHayvan() != null) ? b.getHayvan().getName() : "?";
						System.out.println("  #" + b.getAppId() + " | " + musteriAdi
								+ " → " + hayvanAdi + " | Durum: " + b.getDurum());
					}
				}
				break;
 
			default:
				throw new BarinakIstisnasi("Geçersiz seçim!");
		}
	}
 
	// ═══════════════════════════════════════════════════════════════════════════
	// MÜŞTERİ MODU
	// ═══════════════════════════════════════════════════════════════════════════
 
	public static void musteriModu(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- MÜŞTERİ MODU ---");
		System.out.println("\n1-) Hayvan Sahiplenme Başvurusu Yap");
		System.out.println("2-) Sahiplenilebilecek Hayvanları Görüntüle");
		System.out.println("0-) Ana Menüye Dön");
		System.out.print("Seçiminiz: ");
 
		int secim = scan.nextInt();
		scan.nextLine();
 
		switch (secim) {
			case 0:
				return;
			case 1:
				hayvanSahiplen(scan, yonetici);
				break;
			case 2:
				if (yonetici.tumHayvanlariListele().isEmpty()) {
					System.out.println("Barınakta sahiplenilebilecek hayvan yok.");
				} else {
					System.out.println("\n--- Sahiplenilebilecek Hayvanlar ---");
					for (Hayvan h : yonetici.tumHayvanlariListele()) {
						System.out.println("- ID: " + h.getAnimalId() + " | " + h.getName()
								+ " | " + h.getTur() + " | Sağlık: " + h.getSaglikDurumu());
					}
				}
				break;
			default:
				throw new BarinakIstisnasi("Geçersiz seçim!");
		}
	}
 
	// ═══════════════════════════════════════════════════════════════════════════
	// YENİ KAYIT MODU (Müşteri / Çalışan ekleme)
	// ═══════════════════════════════════════════════════════════════════════════
 
	public static void kayitModu(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- YENİ KAYIT MENÜSÜ ---");
		System.out.println("\n1-) Yeni Müşteri Ekle");
		System.out.println("2-) Yeni Çalışan Ekle (Yönetici / Veteriner / Bakıcı)");
		System.out.println("0-) Ana Menüye Dön");
		System.out.print("Seçiminiz: ");
 
		int secim = scan.nextInt();
		scan.nextLine();
 
		switch (secim) {
			case 0:
				return;
			case 1:
				musteriEkle(scan, yonetici);
				break;
			case 2:
				calisanEkle(scan, yonetici);
				break;
			default:
				throw new BarinakIstisnasi("Geçersiz seçim!");
		}
	}
 
	// ═══════════════════════════════════════════════════════════════════════════
	// ALT İŞLEMLER (private)
	// ═══════════════════════════════════════════════════════════════════════════
 
	// ── Hayvan Sahiplenme ────────────────────────────────────────────────────────
 
	private static void hayvanSahiplen(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- YENİ HAYVAN SAHİPLENME KAYDI ---\n");
 
		if (yonetici.tumMusterileriListele().isEmpty()) {
			throw new BarinakIstisnasi("Sistemde kayıtlı müşteri yok.");
		}
 
		System.out.print("Adınız: ");
		String ad = scan.nextLine();
 
		if (ad == null || ad.trim().isEmpty()) {
			throw new BarinakIstisnasi("Müşteri ismi boş bırakılamaz!");
		}
 
		Musteri aktifMusteri = null;
		for (Musteri m : yonetici.tumMusterileriListele()) {
			if (m.getName().equalsIgnoreCase(ad.trim())) {
				aktifMusteri = m;
				break;
			}
		}
 
		if (aktifMusteri == null) {
			throw new BarinakIstisnasi("'" + ad + "' isimli müşteri sistemde bulunamadı.");
		}
 
		if (yonetici.tumHayvanlariListele().isEmpty()) {
			throw new BarinakIstisnasi("Barınakta sahiplenilebilecek hayvan bulunmamakta.");
		}
 
		System.out.println("\n--- Sahiplenilebilecek Hayvanlar ---");
		for (Hayvan h : yonetici.tumHayvanlariListele()) {
			System.out.println("- " + h.getName() + " | Durumu: " + h.getSaglikDurumu()
					+ " | ID: " + h.getAnimalId());
		}
 
		System.out.print("\nSahiplenmek istediğiniz hayvanın ID numarası: ");
		int arananId = scan.nextInt();
		scan.nextLine();
 
		Hayvan secilenHayvan = null;
		for (Hayvan h : yonetici.tumHayvanlariListele()) {
			if (h.getAnimalId() == arananId) {
				secilenHayvan = h;
				break;
			}
		}
 
		if (secilenHayvan == null) {
			throw new BarinakIstisnasi("Bu ID numarasına sahip bir hayvan bulunamadı!");
		}
 
		basvuruSayaci++;
		SahiplenmeBasvurusu yeniBasvuru = new SahiplenmeBasvurusu(
				basvuruSayaci, "BEKLEMEDE", new Date(), aktifMusteri, secilenHayvan
		);
		aktifMusteri.basvuruYap();
		yonetici.basvuruKaydet(yeniBasvuru);
	}
 
	// ── Başvuru Onayla / Reddet ──────────────────────────────────────────────────
 
	private static void basvuruYonet(Scanner scan, BarinakYoneticisi yonetici, Calisan yetkili) throws Exception {
 
		List<SahiplenmeBasvurusu> bekleyenler = yonetici.bekleyenBasvurular();
 
		if (bekleyenler.isEmpty()) {
			System.out.println("Beklemede olan başvuru bulunmamaktadır.");
			return;
		}
 
		System.out.println("\n--- Bekleyen Başvurular ---");
		for (SahiplenmeBasvurusu b : bekleyenler) {
			String musteriAdi = (b.getMusteri() != null) ? b.getMusteri().getName() : "Bilinmiyor";
			String hayvanAdi = (b.getHayvan() != null) ? b.getHayvan().getName() : "Bilinmiyor";
			int hayvanId = (b.getHayvan() != null) ? b.getHayvan().getAnimalId() : -1;
 
			System.out.println("  Başvuru #" + b.getAppId()
					+ " | Müşteri: " + musteriAdi
					+ " | Hayvan: " + hayvanAdi + " (ID: " + hayvanId + ")"
					+ " | Tarih: " + b.getTarih());
		}
 
		System.out.print("\nİşlem yapmak istediğiniz başvuru numarası: ");
		int secilen = scan.nextInt();
		scan.nextLine();
 
		SahiplenmeBasvurusu hedefBasvuru = yonetici.basvuruBul(secilen);
 
		if (!hedefBasvuru.getDurum().equalsIgnoreCase("BEKLEMEDE")) {
			throw new BarinakIstisnasi("Bu başvuru zaten işlenmiş. Durumu: " + hedefBasvuru.getDurum());
		}
 
		System.out.println("\n1-) Onayla");
		System.out.println("2-) Reddet");
		System.out.print("Seçiminiz: ");
 
		int islem = scan.nextInt();
		scan.nextLine();
 
		switch (islem) {
			case 1:
				yonetici.basvuruOnayla(hedefBasvuru, yetkili);
				break;
			case 2:
				yonetici.basvuruReddet(hedefBasvuru, yetkili);
				break;
			default:
				throw new BarinakIstisnasi("Geçersiz seçim! 1 (Onayla) veya 2 (Reddet) giriniz.");
		}
	}
 
	// ── Sağlık Kaydı Ekleme ──────────────────────────────────────────────────────
 
	private static void saglikKaydiEkle(Scanner scan, BarinakYoneticisi yonetici,
			Veteriner veteriner, Hayvan hedefHayvan) throws Exception {
 
		System.out.print("Tanı: ");
		String tani = scan.nextLine();
		if (tani == null || tani.trim().isEmpty()) {
			throw new BarinakIstisnasi("Tanı boş bırakılamaz!");
		}
 
		System.out.print("Aşı: ");
		String asi = scan.nextLine();
		if (asi == null || asi.trim().isEmpty()) {
			throw new BarinakIstisnasi("Aşı bilgisi boş bırakılamaz!");
		}
 
		SaglikKaydi yeniKayit = new SaglikKaydi(0, hedefHayvan.getAnimalId(), new Date(), tani, asi);
		yonetici.saglikKaydiEkle(yeniKayit, hedefHayvan, veteriner);
	}
 
	// ── Hayvan Ekleme ────────────────────────────────────────────────────────────
 
	private static void hayvanEkle(Scanner scan, BarinakYoneticisi yonetici, Calisan aktifCalisan) throws Exception {
 
		System.out.println("\n--- YENİ HAYVAN KAYDI ---\n");
		System.out.println("1-) Köpek");
		System.out.println("2-) Kedi");
		System.out.print("Seçiminiz: ");
 
		int secim = scan.nextInt();
		scan.nextLine();
 
		if (secim != 1 && secim != 2) {
			throw new BarinakIstisnasi("Geçersiz seçim! 1 (Köpek) veya 2 (Kedi) giriniz.");
		}
 
		String hitap = (secim == 1) ? "Köpeğin" : "Kedinin";
 
		System.out.print(hitap + " ID'si: ");
		int animalId = scan.nextInt();
		scan.nextLine();
 
		if (animalId <= 0) {
			throw new BarinakIstisnasi("Hayvan ID'si sıfırdan büyük olmalıdır.");
		}
 
		for (Hayvan h : yonetici.tumHayvanlariListele()) {
			if (h.getAnimalId() == animalId) {
				throw new BarinakIstisnasi(animalId + " ID numarasına sahip hayvan zaten bulunmaktadır.");
			}
		}
 
		System.out.print(hitap + " ismi: ");
		String name = scan.nextLine();
		if (name == null || name.trim().isEmpty()) {
			throw new BarinakIstisnasi("Hayvan ismi boş bırakılamaz!");
		}
 
		System.out.print(hitap + " türü: ");
		String tur = scan.nextLine();
		if (tur == null || tur.trim().isEmpty()) {
			throw new BarinakIstisnasi("Hayvan türü boş bırakılamaz!");
		}
 
		System.out.print(hitap + " yaşı: ");
		int yas = scan.nextInt();
		scan.nextLine();
 
		if (yas < 0 || yas > 30) {
			throw new BarinakIstisnasi("Hayvan yaşı 0-30 aralığında olmalıdır!");
		}
 
		System.out.print(hitap + " sağlık durumu: ");
		String saglikDurumu = scan.nextLine();
		if (saglikDurumu == null || saglikDurumu.trim().isEmpty()) {
			throw new BarinakIstisnasi("Sağlık durumu boş bırakılamaz!");
		}
 
		System.out.print(hitap + " ırkı: ");
		String irk = scan.nextLine();
		if (irk == null || irk.trim().isEmpty()) {
			throw new BarinakIstisnasi("Hayvan ırkı boş bırakılamaz!");
		}
 
		Hayvan yeniHayvan;
		if (secim == 1) {
			yeniHayvan = new Kopek(animalId, name, tur, yas, saglikDurumu, irk);
		} else {
			yeniHayvan = new Kedi(animalId, name, tur, yas, saglikDurumu, irk);
		}
 
		yonetici.hayvanEkle(yeniHayvan, aktifCalisan);
 
		// İlk sağlık kaydı opsiyonel
		System.out.print("\nİlk sağlık kaydı eklemek ister misiniz? (E/H): ");
		String ekle = scan.nextLine().trim();
 
		if (ekle.equalsIgnoreCase("E")) {
			System.out.print("İlk muayene tanısı: ");
			String tani = scan.nextLine();
 
			System.out.print("Yapılan aşı: ");
			String asi = scan.nextLine();
 
			SaglikKaydi ilkKayit = new SaglikKaydi(0, animalId, new Date(),
					(tani == null || tani.isEmpty()) ? "İlk muayene" : tani,
					(asi == null || asi.isEmpty()) ? "Yok" : asi);
 
			yeniHayvan.saglikKaydiEkle(ilkKayit);
			VeritabaniIslemleri.saglikKaydiEkle(ilkKayit);
			System.out.println("[KAYIT] " + name + " için ilk sağlık kaydı oluşturuldu.");
		}
	}
 
	// ── Müşteri Ekleme ───────────────────────────────────────────────────────────
 
	private static void musteriEkle(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- YENİ MÜŞTERİ KAYDI ---\n");
 
		System.out.print("Müşteri TC numarası: ");
		long id = scan.nextLong();
		scan.nextLine();
 
		tcDogrula(id);
 
		for (Musteri mevcutMusteri : yonetici.tumMusterileriListele()) {
			if (mevcutMusteri.getId() == id) {
				throw new BarinakIstisnasi(id + " numarasına sahip müşteri zaten bulunmakta!");
			}
		}
 
		System.out.print("Müşteri ismi: ");
		String isim = scan.nextLine();
		if (isim == null || isim.trim().isEmpty()) {
			throw new BarinakIstisnasi("Müşteri ismi boş bırakılamaz!");
		}
 
		System.out.print("Müşteri maili: ");
		String email = scan.nextLine();
		if (email == null || email.trim().isEmpty()) {
			throw new BarinakIstisnasi("E-posta adresi boş bırakılamaz!");
		}
		for (Musteri mevcutMusteri : yonetici.tumMusterileriListele()) {
			if (mevcutMusteri.getEmail().equalsIgnoreCase(email)) {
				throw new BarinakIstisnasi(email + " mailine sahip müşteri zaten bulunmakta!");
			}
		}
 
		System.out.print("Müşteri şifresi: ");
		String password = scan.nextLine();
		if (password == null || password.trim().isEmpty()) {
			throw new BarinakIstisnasi("Şifre boş bırakılamaz!");
		}
 
		System.out.print("Müşterinin müşteri ID numarası: ");
		String customerId = scan.nextLine();
		if (customerId == null || customerId.trim().isEmpty()) {
			throw new BarinakIstisnasi("Müşteri kodu boş bırakılamaz!");
		}
		for (Musteri mevcutMusteri : yonetici.tumMusterileriListele()) {
			if (mevcutMusteri.getCustomerId().equalsIgnoreCase(customerId)) {
				throw new BarinakIstisnasi(customerId + " ID numarasına sahip müşteri zaten bulunmakta!");
			}
		}
 
		System.out.print("Müşteri adresi: ");
		String address = scan.nextLine();
 
		System.out.print("Müşteri telefon numarası: ");
		String phone = scan.nextLine();
		if (!phone.matches("^[0-9]{10,11}$")) {
			throw new BarinakIstisnasi("Telefon numarası sadece rakamlardan oluşmalı ve 10-11 haneli olmalıdır!");
		}
		for (Musteri mevcutMusteri : yonetici.tumMusterileriListele()) {
			if (mevcutMusteri.getPhone().equals(phone)) {
				throw new BarinakIstisnasi(phone + " telefon numarasına sahip müşteri zaten bulunmakta!");
			}
		}
 
		Musteri yeniKayit = new Musteri(id, isim, email, password, customerId, address, phone);
		yonetici.musteriEkle(yeniKayit);
	}
 
	// ── Çalışan Ekleme ───────────────────────────────────────────────────────────
 
	private static void calisanEkle(Scanner scan, BarinakYoneticisi yonetici) throws Exception {
 
		System.out.println("\n--- YENİ ÇALIŞAN KAYDI ---\n");
		System.out.println("Rol seçimi:");
		System.out.println("1-) Yönetici");
		System.out.println("2-) Veteriner");
		System.out.println("3-) Bakıcı");
		System.out.print("Seçiminiz: ");
 
		int rolSecim = scan.nextInt();
		scan.nextLine();
 
		if (rolSecim < 1 || rolSecim > 3) {
			throw new BarinakIstisnasi("Geçersiz rol seçimi! 1, 2 veya 3 giriniz.");
		}
 
		System.out.print("Çalışan TC numarası: ");
		long id = scan.nextLong();
		scan.nextLine();
 
		tcDogrula(id);
 
		for (Calisan mevcutCalisan : yonetici.tumCalisanlariListele()) {
			if (mevcutCalisan.getId() == id) {
				throw new BarinakIstisnasi(id + " TC numaralı çalışan zaten bulunmakta!");
			}
		}
 
		System.out.print("Çalışan ismi: ");
		String isim = scan.nextLine();
		if (isim == null || isim.trim().isEmpty()) {
			throw new BarinakIstisnasi("Çalışan ismi boş bırakılamaz!");
		}
 
		System.out.print("Çalışan maili: ");
		String email = scan.nextLine();
		if (email == null || email.trim().isEmpty()) {
			throw new BarinakIstisnasi("E-posta adresi boş bırakılamaz!");
		}
		for (Calisan mevcutCalisan : yonetici.tumCalisanlariListele()) {
			if (mevcutCalisan.getEmail().equalsIgnoreCase(email)) {
				throw new BarinakIstisnasi(email + " mailine sahip çalışan zaten bulunmakta!");
			}
		}
 
		System.out.print("Çalışan şifresi: ");
		String password = scan.nextLine();
		if (password == null || password.trim().isEmpty()) {
			throw new BarinakIstisnasi("Şifre boş bırakılamaz!");
		}
 
		System.out.print("Çalışanın çalışan Id numarası: ");
		String employeeId = scan.nextLine();
		if (employeeId == null || employeeId.trim().isEmpty()) {
			throw new BarinakIstisnasi("Çalışan kodu boş bırakılamaz!");
		}
		for (Calisan mevcutCalisan : yonetici.tumCalisanlariListele()) {
			if (mevcutCalisan.getEmployeeId().equalsIgnoreCase(employeeId)) {
				throw new BarinakIstisnasi(employeeId + " ID numarasına sahip çalışan zaten bulunmakta!");
			}
		}
 
		System.out.print("Çalışanın vardiyası: ");
		String shift = scan.nextLine();
 
		Calisan yeniCalisan;
		switch (rolSecim) {
			case 1:
				yeniCalisan = new Yonetici(id, isim, email, password, employeeId, shift);
				break;
 
			case 2:
				System.out.print("Veterinerin uzmanlık alanı: ");
				String uzmanlik = scan.nextLine();
				if (uzmanlik == null || uzmanlik.trim().isEmpty()) {
					throw new BarinakIstisnasi("Uzmanlık alanı boş bırakılamaz!");
				}
				yeniCalisan = new Veteriner(id, isim, email, password, employeeId, shift, uzmanlik);
				break;
 
			case 3:
				System.out.print("Bakıcının sorumlu olduğu bölge: ");
				String bolge = scan.nextLine();
				if (bolge == null || bolge.trim().isEmpty()) {
					throw new BarinakIstisnasi("Sorumlu bölge boş bırakılamaz!");
				}
				yeniCalisan = new Bakici(id, isim, email, password, employeeId, shift, bolge);
				break;
 
			default:
				throw new BarinakIstisnasi("Geçersiz rol seçimi.");
		}
 
		yonetici.calisanEkle(yeniCalisan);
	}
 
	// ── Yardımcı: Çalışan Doğrulama ──────────────────────────────────────────────
 
	private static Calisan calisanDogrula(Scanner scan, BarinakYoneticisi yonetici) throws BarinakIstisnasi {
		if (yonetici.tumCalisanlariListele().isEmpty()) {
			throw new BarinakIstisnasi("Sistemde kayıtlı çalışan yok.");
		}
 
		System.out.print("Çalışan ID numarası: ");
		String calisanId = scan.nextLine().trim();
 
		if (calisanId.isEmpty()) {
			throw new BarinakIstisnasi("Çalışan ID boş olamaz.");
		}
 
		for (Calisan c : yonetici.tumCalisanlariListele()) {
			if (c.getEmployeeId().equals(calisanId)) {
				return c;
			}
		}
 
		throw new BarinakIstisnasi("'" + calisanId + "' ID'li çalışan bulunamadı.");
	}
 
	// ── TC Kimlik Doğrulama ──────────────────────────────────────────────────────
 
	private static void tcDogrula(long id) throws BarinakIstisnasi {
		String metin = String.valueOf(Math.abs(id));
 
		if (metin.length() != 11) {
			throw new BarinakIstisnasi("TC numarası 11 haneli olmalıdır. Girilen: " + metin.length() + " hane.");
		}
 
		int toplam = 0;
		for (int i = 0; i < 10; i++) {
			toplam += Character.getNumericValue(metin.charAt(i));
		}
 
		int onBirinciRakam = Character.getNumericValue(metin.charAt(10));
 
		if (onBirinciRakam % 2 != 0) {
			throw new BarinakIstisnasi("TC numarasının son hanesi çift olmalıdır.");
		}
 
		if (toplam % 10 != onBirinciRakam) {
			throw new BarinakIstisnasi("Geçersiz TC Kimlik Numarası.");
		}
	}
}